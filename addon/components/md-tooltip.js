...
var MdTooltip = Component.extend(LayoutRules, {

  tagName: 'md-tooltip',

  utilityService: service('utility'),
  ...
  showTooltip: observer('visible', function () {
    if (!this.get('visible')) {
      return;
    }
    ...
    if (isPresent(computedStyles.display) && computedStyles.display === 'none') {
      this.$().detach();
      return;
    }
  ...
  setupComponent() {
    ...
    this.background = $(this.$()[0].getElementsByClassName('md-background')[0]);
    this.content = $(this.$()[0].getElementsByClassName('md-content')[0]);
    this.current = this.getNearestContentElement();
    this.tooltipParent = $(this.current || document.body);

    schedule('afterRender', () => this.bindEvents());
  },
