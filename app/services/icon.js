...
function Icon(el, config) {
  if (el.tagName != 'svg') {
    el = $('<svg xmlns="http://www.w3.org/2000/svg">').append(el)[0];
  }
...
var IconService = Service.extend({

  iconPromiseCache: EmberMap.create(),
  ...
  getIcon: function (id) {
    ...
    // if already loaded and cached, use a clone of the cached icon.
    if (config[id]) {
      return EmberPromise.resolve(config[id].clone());
    }
    ...
    return this.cacheIconPromise(id, this.loadByID(id)
      .catch(bind(this, this.loadFromIconSet))
  ...
  loadByID: function (id) {
    ...
    return !iconConfig ? EmberPromise.reject(id) : this.loadByURL(iconConfig.url).then(function (icon) {
  ...
  loadFromIconSet: function (id) {
    ...
    function extractFromSet(svg) {

      if (!self.templateCache[iconSetConfig.url]) {
        self.templateCache[iconSetConfig.url] = svg;
      }

      var iconName = id.slice(id.lastIndexOf(':') + 1);
      var icon = svg.querySelector('#' + iconName);
      return !icon ? EmberPromise.reject(id) : new Icon(icon, iconSetConfig);
    }

    return !iconSetConfig ? EmberPromise.reject(id) : this.loadByURL(iconSetConfig.url).then(extractFromSet);
  },
  ...
  loadByURL: function (url) {
    ...
    if (this.templateCache[url]) {
      req = EmberPromise.resolve(this.templateCache[url]);
    } else {
      req = request(url, {
        dataType: 'text'
      });
  ...
  cacheIconPromise: function (id, promise) {
    ...
    iconCache.set(id, promise.then(function (icon) {
      ...
      iconCache.set(id, EmberPromise.resolve(icon));
